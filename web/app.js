...
const labels={'awaiting-image':'Потрібне зображення','waiting-parent':'Очікує Reel',draft:'Чернетка',ready:'Готово',posted:'Опубліковано',error:'Потрібна дія',skipped:'Пропущено',interrupted:'Перезапуск',queued:'У черзі',running:'Готується',done:'Готово'};
...
function renderStudio(){const prompts=(state.data.plan?.items||[]).filter(i=>i.purpose==='useful');$('#studio-prompts').innerHTML=prompts.length?prompts.map(i=>`<p>${escape(state.date)} · ${escape(i.title||i.label)}<br><button type="button" class="secondary" data-open-item="${i.id}">Відкрити промпт і завантаження</button></p>`).join(''):'<p class="hint">Спочатку сформуй день у календарі.</p>';const list=state.data.assets.filter(a=>!a.productId&&['ai','editorial'].includes(a.source));$('#studio-grid').innerHTML=list.length?list.map(a=>`<article class="product-card"><div class="product-photo"><img src="${url(a.id)}/thumbnail" alt="Редакційний матеріал" loading="lazy"></div><div class="product-info"><h3>${escape(a.name)}</h3><p>${a.source==='ai'?'ШІ ілюстрація':'Власний матеріал'}</p><a class="secondary" href="${url(a.id)}?download=1">↓ Завантажити</a></div></article>`).join(''):'<p class="hint">Тут з’являться твої ілюстрації та редакційні відео.</p>';}
function fillSettings(){const d=state.data;if(!d)return;const f=$('#settings-form');for(const [key,value] of Object.entries(d.settings)){const input=f.elements.namedItem(key);if(input){if(input.type==='checkbox')input.checked=value;else input.value=value;}}
  $('#setup-status').innerHTML=[['Тексти й промпти',d.setup.text],['Озвучка · '+d.setup.voiceProvider,d.setup.voice],['Google Drive',d.setup.drive]].map(([name,ready])=>`<div class="setup-row"><span>${name}</span><span class="${ready?'status-ok':'status-off'}">${ready?'● Налаштовано':'○ Потрібен ключ'}</span></div>`).join('');
  ...
  $('#usage-details').textContent=`$${d.usage.reserve.toFixed(2)} із $${d.settings.dailyBudget.toFixed(2)}. Текстових запитів: ${d.usage.counts.text||0}; малювання через API вимкнено; символів озвучки: ${d.usage.counts.voice||0}.`;
}
...
function openItem(id){const i=item(id);if(!i)return;state.editingItem=id;const f=$('#item-form');f.reset();for(const key of ['id','title','caption','pollQuestion','imagePrompt'])f.elements.namedItem(key).value=i[key]||'';
  ...
  $('#item-goal').textContent=i.goal||'';$('#illustration-workflow').hidden=i.purpose!=='useful';$('#illustration-workflow').open=['draft','awaiting-image','error'].includes(i.status);$('#drawing-prompt').value=i.drawingPrompt||'Промпт з’явиться після підготовки текстів.';$('#copy-image-prompt').disabled=!i.drawingPrompt;$('#illustration-progress').textContent='';$('#shared-illustration').hidden=!i.imageSourceId||i.imageSourceId===i.id;$('#raw-image-prompt').hidden=i.purpose!=='useful';
  const preview=i.outputIds.map(asset).find(a=>a?.kind==='video')||asset(i.coverId)||i.outputIds.map(asset).find(a=>a?.kind==='image');
  $('#item-media').innerHTML=preview?(preview.kind==='video'?`<video src="${url(preview.id)}" ${i.coverId?`poster="${url(i.coverId)}"`:""} controls playsinline preload="metadata"></video>`:`<img src="${url(preview.id)}" alt="${escape(i.title)}">`):`<div class="empty-state"><div class="empty-icon">✧</div><p>${escape(['awaiting-image','waiting-parent'].includes(i.status)?i.notes?.[0]:'Тут буде готовий матеріал')}</p></div>`;
  ...
  const p=product(i.productId);$('#product-facts').textContent=p?(i.kind==='story'?'Дані товару для довідки (не додаються до короткої сторіз):\n'+i.productFacts:i.kind==='reel'?'Характеристики товару:\n'+i.productFacts+'\n\nДля короткого підпису Reel програма додає ціну, артикул і розміри.':'У підписі каруселі — усі заповнені характеристики товару.') : '';$('#full-caption-section').hidden=!p||i.kind==='story';$('#full-caption-preview').value=i.fullCaption||'';$('#copy-product-facts').hidden=!p;
  ...
  const locked=i.status==='posted'||busy();[...f.elements].forEach(el=>{if(el.tagName!=='BUTTON')el.disabled=locked&&!el.readOnly;});$('#mark-posted').disabled=i.status!=='ready'||busy();$('#regen-text').disabled=locked;$('#regen-all').disabled=locked;$('#rerender').disabled=locked;$('#skip-item').disabled=locked;f.querySelector('[type=submit]').disabled=locked;
  $('#item-illustration-upload').disabled=locked||i.status==='skipped'||!i.drawingPrompt;
...
$('#item-form').onsubmit=e=>{e.preventDefault();action(async()=>{await saveItem();await refresh();openItem(state.editingItem);toast('Правки збережено');});};
$('#copy-product-facts').onclick=()=>action(()=>copy(item(state.editingItem).productFacts));
...
$('#copy-image-prompt').onclick=()=>action(async()=>{if(item(state.editingItem).status!=='posted'&&!busy()){await saveItem();await refresh();}$('#drawing-prompt').value=item(state.editingItem).drawingPrompt||'';await copy(item(state.editingItem).drawingPrompt);});$('#regen-all').onclick=()=>action(()=>regenerate('all'));$('#regen-text').onclick=()=>action(()=>regenerate('text'));$('#rerender').onclick=()=>action(()=>regenerate('render'));
...
$('#item-illustration-upload').onchange=e=>action(async()=>{const file=e.target.files[0];if(!file||state.formBusy)return;state.formBusy=true;e.target.disabled=true;$('#illustration-progress').textContent='Завантажую зображення…';try{await saveItem();await api(`/api/plans/${state.date}/items/${state.editingItem}/illustration?name=${encodeURIComponent(file.name)}`,{method:'POST',raw:file});$('#item-dialog').close();await refresh();toast('Зображення збережено. Готую Reel та опитування.');}finally{state.formBusy=false;e.target.value='';e.target.disabled=busy()||item(state.editingItem)?.status==='posted';$('#illustration-progress').textContent='';}});
