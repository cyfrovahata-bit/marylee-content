...
function openItem(id){const i=item(id);if(!i)return;state.editingItem=id;const f=$('#item-form');f.reset();for(const key of ['id','title','caption','pollQuestion','imagePrompt'])f.elements.namedItem(key).value=i[key]||'';
  ...
  const locked=i.status==='posted'||busy();[...f.elements].forEach(el=>{if(el.tagName!=='BUTTON')el.disabled=locked;});$('#mark-posted').disabled=i.status!=='ready'||busy();$('#regen-text').disabled=locked;$('#rerender').disabled=locked;$('#skip-item').disabled=locked;f.querySelector('[type=submit]').disabled=locked;
...
$('#copy-image-prompt').onclick=()=>action(()=>copy($('#item-form').elements.imagePrompt.value));$('#regen-text').onclick=()=>action(()=>regenerate('text'));$('#rerender').onclick=()=>action(()=>regenerate('render'));
