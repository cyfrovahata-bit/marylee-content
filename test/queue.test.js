...
function fakeDrive(s){
  const roots={id:'drive',root:'marylee-root',input:'marylee-input',output:'marylee-output',briefs:'marylee-briefs',results:'marylee-results',prompts:'marylee-prompts',templateVersion:2,queueSheetId:'queue-sheet',templates:{'reel-tip':{id:'tip-template'},poll:{id:'poll-template'}}};s.put('integration',roots);
  const rows=[],uploads=[];
...
test('five products fill five one-product days without duplicated reservations',async t=>{
  const s=await fixture(t),batch=createBatch(s),plans=s.list('plan').sort((a,b)=>a.id.localeCompare(b.id));
  assert.equal(batch.products,5);assert.deepEqual(plans.map(p=>p.productIds.length),[1,1,1,1,1]);assert.equal(batch.dates[0],shiftDate(kyivToday(),1));
  assert.equal(plans[2].items.find(i=>i.id==='carousel').productId,plans[2].items.find(i=>i.id==='evening').productId);
  assert.deepEqual(createBatch(s).dates,[]);assert.equal(s.activeJobs().length,5);
});
test('queue export is idempotent; stale generations cancel and leases require manual retry',async t=>{
  ...
  await q.ensure(p);await q.ensure(p);assert.equal(d.rows.length,1);assert.equal(d.uploads.length,1);
  ...
  assert.equal(s.get('image-job',old).status,'CANCELLED');assert.equal(d.rows.length,2);assert.equal(d.rows[1][10],2);
});
...
test('validated ZIP imports four scenes once, preserves checked script through API copy and renders voiced Reel', {timeout:120000},async t=>{
  ...
  for(const [n,name] of names.entries())await run('ffmpeg',['-y','-f','lavfi','-i',`color=c=${['red','green','blue','yellow'][n]}:s=1080x1920`,'-frames:v','1','-threads','1',path.join(bundleDir,name)]);
  ...
  const current=s.get('plan',plan.id),item=current.items[0];assert.equal(item.selectedAssetIds.length,4);assert.equal(current.items.some(i=>i.kind==='story'),false);
