...
export function copySchema(items,store=null) {
  ...
  const properties=Object.fromEntries(items.map(item=>[item.id,{
    ...itemSchema,properties:{...itemSchema.properties,
      ...
      detailFocus:{type:'string',enum:item.purpose==='detail'?['full','upper','lower']:['full']},
...
Не роби висновків про країну виробництва, бренд або чистоту складу з того, що магазин продає в Україні. Не додавай «український бренд», «натуральний» чи подібні твердження в тексти, keywords і hashtags без прямого підтвердження в даних товару. Для редакційної ілюстрації використовуй лише теги про стиль, одяг і кольори.
